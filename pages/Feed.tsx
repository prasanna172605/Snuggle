...
}> = ({ onUserClick, onStoryClick, storyUsers, storiesByUser }) => {
...
const Feed: React.FC<FeedProps> = ({ currentUser, onUserClick }) => {
  ...
  useEffect(() => {
      ...
      const unsubscribe = DBService.subscribeToPosts(async (newPosts) => {
          setPosts(newPosts);
          
          // Fetch users for posts if not already loaded
          const uIds = new Set(newPosts.map(p => p.userId));
          ...
          try {
            ...
            console.error("Error fetching data for feed", e);
          }
          setLoading(false);
      });
